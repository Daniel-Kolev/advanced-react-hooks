...
useReducer can help us move the state management logic out of the component.
Also it's useful when you have different state variables that change at the same
time.

Make sure to return the updated state from the reducer function

Also you can pass in a third parameter to useReducer which takes the second
parameter from useReducer to lazy intialize the state
